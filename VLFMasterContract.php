...
function myDebug()
{
	return false;
		...
		echo '
			<form action="VLFMasterContract.php" method="post">
				Doc name: <input type="text" name="doc_name" value="VLFMasterContractBuilder"><br>
	...
	function showLogo($imageURL, $width=7) {
	 ...
	 function showFees($mode = 'contract') {
		...
		GLOBAL $dev_mode ;
		$contract_fees = array ( "ATTORNEYS FEES FOR PERMANENT RESIDENT VISA – Immediate Relative of U.S. Citizen "=>"1500",
		"ADMINISTRATIVE FEES / SHIPPING & HANDLING"=>"450",
		"USCIS FILING FEES – FORM I-130  $535.00 PER PERSON"=>"535",
		"USCIS FILING FEES – FORM I-485  $1225.00 PER PERSON"=>"1225",
		"APPLICATION FOR TRAVEL – FORM I-131"=>"0",
		"APPLICATION FOR EMPLOYMENT & BIOMETRICS/FINGERPRINTS"=>"0" );
		...
		$tableF->writeToCell(1,2,'<b>AMOUNT ( IN USD )</b>') ;
		//$cellAmt = $tableF->getCell(1, 2) ;
		//$cellAmt->setTextAlignment(TEXT_ALIGN_RIGHT) ;
		$currentRow = 2 ;

		while($feesRow = each($contract_fees)) {
			$tableF->addRow() ;
			$tableF->writeToCell($currentRow, 1, $feesRow[0]) ;
			if (is_numeric($feesRow[1])) {
				$tableF->writeToCell($currentRow, 2,  $feesRow[1]) ;
				$fees_total += floatval($feesRow[1]) ;
			}
			else {
				$tableF->writeToCell($currentRow, 2, $feesRow[1]) ;
			}
		...
		$tableF->writeToCell($currentRow, 2,  '<b>' . $fees_total . '</b>' ) ;
...
$getFieldLCarray = array ( "beneficiary_name_first"=>"Nilay", "beneficiary_name_last"=>"Jayswal", "beneficiary_contact_email"=>"nilayjayswalnj@gmail.com", "beneficiary_contact_phone_daytime"=>"+91 8826568459" );

		
		return $getFieldLCarray[$linknode];
	}
	
	function getContractFees($linknode, $returnAll = False)
	{
		global $getContractFeesarray;

$getContractFeesarray = array ( "fee_permanent_resident_visa"=>"1500.00", "fee_administrative"=>"450.00", "fee_formI130"=>"535.00", "fee_formI485"=>"1225.00", "fee_formI131"=>"0.00", "fee_employment_biometrics"=>"0.00" );

		
		return $getContractFeesarray[$linknode];
	}
