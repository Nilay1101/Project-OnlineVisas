 ...
 * @version    $Id: Image.php 1031 2017-03-17 07:06:12Z timo.scholz $
 */
...
abstract class SetaPDF_Core_Image
{
    ...
    static public function getType(SetaPDF_Core_Reader_ReaderInterface $reader)
    {
        ...
        switch (1) {
            ...
            case strlen($bytes) > 4 && $bytes[0] == 'M' && $bytes[1] == 'M' && $bytes[2] == "\x00" && $bytes[3] == "\x2A":
            case strlen($bytes) > 4 && $bytes[0] == 'I' && $bytes[1] == 'I' && $bytes[2] == "\x2A" && $bytes[3] == "\x00":
                $type = self::TYPE_TIFF;
                break;
        }
